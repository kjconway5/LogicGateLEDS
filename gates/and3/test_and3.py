...
async def three_input_test(dut, a, b, d):
    ...
    assert_resolvable(dut.c_o)
    assert dut.c_o.value == (dut.a_i.value & dut.b_i.value & dut.d_i.value) , f"Incorrect Result: {dut.a_i.value} & {dut.b_i.value} & {dut.d_i.value} != {dut.a_i.value & dut.b_i.value & dut.d_i.value}. Got: {dut.c_o.value} at Time {get_sim_time(units='ns')}ns."
